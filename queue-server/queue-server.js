...
caps.on('connection', socket => {
  ...
  socket.on('pickup', payload => {
    ...
    caps.emit('pickup', {id:id, payload: undeliveredQueue.messages[id]});
  });
  ...
  socket.on('in-transit', payload => { 
    ...
    caps.to(payload.store).emit('in-transit', {id:id, payload: undeliveredQueue.messages[id]}); 
  });
  ...
  socket.on('delivered', payload => {
    ...
    caps.to(payload.store).emit('delivered',{id:id, payload: undeliveredQueue.messages[id]});
  });

  socket.on('get-all', payload => {

    Object.keys(undeliveredQueue.messages).forEach(id=> {
      socket.emit('message', {id:id, payload: undeliveredQueue.messages[id]});
    });
